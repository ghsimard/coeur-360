 ...
 */
export function corregirPesos(pesos: Readonly<Record<RolObservador, number>>): Record<RolObservador, number> {
...
export function prepararInstrumento(crudo: InstrumentoCrudo): Instrumento {
  const gestiones = new Set(crudo.gestiones.map((g) => g.clave));
  const ordenCompetencia = new Map<string, number>();
  ...
  const itemPorNumero = new Map<number, ItemDef>();
  ...
  for (const it of crudo.items) {
    ...
    if (itemPorNumero.has(it.numero)) throw new InstrumentoInvalido(`Número de pregunta duplicado: ${it.numero}`);
    if (claves.has(it.clave)) throw new InstrumentoInvalido(`Clave de ítem duplicada: ${it.clave}`);
  ...
  const items = [...crudo.items].sort(
    (a, b) =>
      (ordenCompetencia.get(a.competencia) ?? 0) - (ordenCompetencia.get(b.competencia) ?? 0) ||
      a.clave.localeCompare(b.clave, "en", { numeric: true }),
  );

  const originales = new Map<string, Record<RolObservador, number>>();
  for (const p of crudo.pesos) {
    if (!claves.has(p.item)) throw new InstrumentoInvalido(`Peso para un ítem desconocido: ${p.item}`);
    if (!ROLES_OBSERVADOR.includes(p.rol)) throw new InstrumentoInvalido(`Rol de observador desconocido: ${p.rol}`);
    if (!(p.peso >= 0)) throw new InstrumentoInvalido(`Peso inválido para ${p.item}/${p.rol}: ${p.peso}`);
    const fila = originales.get(p.item) ?? { coor: 0, doce: 0, admi: 0, acud: 0, estu: 0 };
    fila[p.rol] = p.peso;
    originales.set(p.item, fila);
  }
  const pesos = new Map<string, Record<RolObservador, number>>();
  for (const it of items) {
    const fila = originales.get(it.clave);
    if (!fila) throw new InstrumentoInvalido(`El ítem ${it.clave} no tiene pesos`);
