...
const FRECUENCIA: Readonly<Record<string, number>> = {
  nunca: 2.5,
  "pocas veces": 5,
  "algunas veces": 7.5,
  siempre: 10,
...
const ACUERDO: Readonly<Record<string, number>> = {
 ...
 */
export function puntajeDeEtiqueta(
  escala: Escala,
  ...
  if (v === undefined) {
    if (estricto) throw new Error(`Etiqueta desconocida para la escala ${escala}: « ${etiqueta} »`);
    return null;
  }
  return v;
}
...
export function etiquetasDeEscala(escala: Escala): string[] {
